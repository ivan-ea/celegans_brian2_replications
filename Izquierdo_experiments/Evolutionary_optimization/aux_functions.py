...
def normalize_trace_avg_unitary(tr, start_point):
    norm_traces = zeros(shape(tr))
    for i in range(shape(tr)[0]):
        mi = min(tr[i][start_point:])
        ma = max(tr[i][start_point:])
        amp=0.5*(ma-mi)
        if amp < 1: amp = 1
        norm_traces[i] = (diff_V(tr[i], 0.5*(ma+mi)))/amp          
    return norm_traces 

def norm_trace_avg_unitary_2points(tr, s1,e1,s2,e2):
    norm_traces = zeros(shape(tr))
    
    for i in range(shape(tr)[0]):
        eval_tr = concatenate([tr[i][s1:e1],tr[i][s2:e2]])
        mi = min(eval_tr)
        ma = max(eval_tr)
        amp=0.5*(ma-mi)
        if amp < 1: amp = 1
